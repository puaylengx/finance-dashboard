<template>
  <div class="min-h-screen bg-bg flex items-center justify-center px-4 py-10">
    ...
    <button
      ...
      :aria-label="
        themeCtrl.theme.value === 'dark' ? 'สลับเป็นโหมดสว่าง' : 'สลับเป็นโหมดมืด'
      "
    ...
    >
      <svg
        v-if="themeCtrl.theme.value === 'dark'"
        width="15"
        height="15"
        viewBox="0 0 24 24"
        fill="none"
    ...
    <div class="w-full max-w-lg animate-fade-in">
      ...
      <div class="text-center mb-8">
        <div
          class="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-accent/15 border border-accent/30 mb-4"
        >
          <svg
            width="22"
            height="22"
        ...
        <p class="text-sm text-muted mt-1">
          {{
            hasMsal
              ? "เข้าสู่ระบบด้วยบัญชี Microsoft องค์กร"
              : "เลือกหน่วยงานและตำแหน่งเพื่อเข้าสู่ระบบ"
          }}
        </p>
      </div>
      ...
      <div
        v-if="sessionExpired"
        class="flex items-start gap-3 rounded-xl border border-yellow-500/30 bg-yellow-500/10 px-4 py-3 mb-2"
      >
        <svg
      ...
      <div class="rounded-2xl border border-border bg-surface p-6 flex flex-col gap-6">
        ...
        <div v-if="hasMsal && isDraft" class="flex items-center gap-3 -my-2">
          ...
          <button
            type="button"
            @click="showDraftForm = !showDraftForm"
        ...
        <template v-if="showDraftForm">
          ...
          <div>
            ...
            <button
              ...
              :class="isFASelected ? 'role-card--active' : ''"
            >
              <div class="flex items-center gap-3">
                <div
                  class="w-8 h-8 rounded-lg flex items-center justify-center shrink-0"
                  :class="isFASelected ? 'bg-accent/25' : 'bg-surface2'"
                >
                  <svg
                    width="14"
                    height="14"
                ...
                <div class="text-left">
                  <div
                    ...
                    :class="isFASelected ? 'text-accent' : 'text-fg'"
                  >
                    FA
                  </div>
                  <div class="text-xs text-muted">
                    Finance &amp; Accounting — Full Access
                  </div>
                </div>
              </div>
              <div
                v-if="isFASelected"
                class="w-4 h-4 rounded-full bg-accent flex items-center justify-center shrink-0"
              >
                <svg
                  width="8"
                  height="8"
            ...
            <div v-if="isFASelected" class="flex gap-2 mt-2 mb-3">
              <button
                v-for="pos in STAFF_POSITIONS"
                :key="pos.value"
                type="button"
                @click="
                  selectedPosition = selectedPosition === pos.value ? null : pos.value
                "
                class="pos-pill"
              ...
              <span
                class="pos-pill text-muted/50 cursor-default border-dashed"
            ...
            <div class="flex gap-2 mb-3">
              <button
                ...
                class="cat-tab flex-1"
                :class="category === cat.value ? 'cat-tab--active' : ''"
              >
                {{ cat.label }}
              </button>
            </div>
            ...
            <div v-if="category === 'division'" class="grid grid-cols-3 gap-2">
              <button
                v-for="d in DIVISION_LIST"
                :key="d.value"
                type="button"
                @click="selectedRole = d.value"
                class="div-card"
                :class="selectedRole === d.value ? 'div-card--active' : ''"
              >
                <div
                  class="text-sm font-bold leading-tight"
                  :class="selectedRole === d.value ? 'text-accent' : 'text-fg'"
                >
                  {{ d.label }}
                </div>
                <div
                  class="text-[10px] leading-tight mt-0.5"
                  :class="selectedRole === d.value ? 'text-accent/70' : 'text-muted'"
                >
                  {{ d.sub }}
                </div>
              </button>
            ...
            <div v-if="category === 'staff'">
              <div class="grid grid-cols-5 gap-2 mb-3">
                <button
                  v-for="s in STAFF_LIST"
                  :key="s.value"
                  type="button"
                  @click="selectedRole = s.value"
                  class="div-card"
                  :class="selectedRole === s.value ? 'div-card--active' : ''"
                >
                  <div
                    class="text-sm font-bold"
                    :class="selectedRole === s.value ? 'text-accent' : 'text-fg'"
                  >
                    {{ s.label }}
                  </div>
                </button>
            ...
            <div v-if="category === 'faculty'">
              ...
              <div class="grid grid-cols-4 gap-2">
                <button
                  v-for="r in FACULTY_ROLES"
                  :key="r.value"
                  type="button"
                  @click="selectedRole = r.value"
                  class="div-card"
                  :class="selectedRole === r.value ? 'div-card--active' : ''"
                >
                  <div
                    class="text-sm font-bold"
                    :class="selectedRole === r.value ? 'text-accent' : 'text-fg'"
                  >
                    {{ r.label }}
                  </div>
                </button>
              ...
              <div v-if="selectedRole" class="mt-2">
                <span
                  class="inline-flex items-center gap-1 text-xs text-accent bg-accent/10 border border-accent/30 rounded-full px-3 py-1"
                >
                  <svg
                    width="10"
                    height="10"
          ...
          <Transition name="slide-up">
            <div v-if="jobTitlePreview">
              ...
              <input
                ...
                autocomplete="off"
                @keydown.enter="handleDraftLogin"
              />
            </div>
          </Transition>
          ...
          <!-- Draft: Error + Submit -->
          <div
            v-if="error && showDraftForm"
            class="text-xs text-danger rounded-lg border border-danger/30 bg-danger/10 px-3 py-2"
          >
            {{ error }}
          </div>
          ...
          <div v-if="jobTitlePreview" class="flex items-center gap-3">
            <div
              class="flex-1 text-xs text-muted bg-surface2 rounded-lg px-3 py-2 font-mono truncate"
            >
              {{ jobTitlePreview }}
            </div>
            <button
...
const jobTitlePreview = computed(() => {
  if (isFASelected.value)
    return selectedPosition.value ? `fa,${selectedPosition.value}` : "fa";
...
async function handleDraftLogin() {
  ...
  loading.value = true;
  try {
    const res = await draftLogin({
      job_title: jobTitlePreview.value,
...
async function fetchJobTitleFromGraph(
  account: import("@azure/msal-browser").AccountInfo
): Promise<string> {
  try {
    ...
    const resp = await fetch("https://graph.microsoft.com/v1.0/me?$select=jobTitle", {
...
async function handleMicrosoftLogin(isRetry = false) {
  ...
  try {
    ...
    auth.setUser(
      session.token,
      session,
      result.account?.username ?? result.account?.name ?? ""
    );
    router.push("/budget");
  } catch (e: unknown) {
    ...
    const backendMsg = (e as { response?: { data?: { detail?: string } } })?.response
      ?.data?.detail;
